...
AFE path used by XiaoZhi on ESP32-S3. It starts capture after local detection
and asks Relay to endpoint the utterance. Relay sends
`turn.input_stop` after speech end, no-speech timeout, or maximum duration.
WakeNet remains available while capturing, waiting, or playing so a second
“你好小智” can cancel the old turn and start a new one after its terminal
response. The BOOT key remains a device-endpointed push-to-talk fallback.
